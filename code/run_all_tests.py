...
def main():
    ...
    for folder in os.listdir('Output\\trees3'):
        path = os.path.join('Output\\trees3', folder)
        if not os.path.isdir(path):
            continue
        for scale in [1,2,3]:
            os.system(f"python paint_to_image.py --image_path ..\\images\\2\\trees3.png --trained_net_dir \"{path}\" --ref_path ..\\images\\paint\\trees1.png --paint_start_scale {scale}")
